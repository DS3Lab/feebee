...
def eval_from_matrices_split(train_features, test_features, train_labels, test_labels):
    ...
    if FLAGS.knn_subtest is not None:
        ...
        test_iterations = (test_features.shape[0] + FLAGS.knn_subtest - 1 ) // FLAGS.knn_subtest
    else:
        test_dividable = True
        test_iterations = 1
    ...
    for i in range(test_iterations):
        if test_iterations == 1:
            ...
            current_pos = i*FLAGS.knn_subtest
            current_samples = min(test_features.shape[0],(i+1)*FLAGS.knn_subtest) - current_pos
        ...
        if FLAGS.knn_subtrain is not None:
            # select per split to nearest k (for max k) train point distances and indices
            train_iterations = (train_features.shape[0] + FLAGS.knn_subtrain -1) // FLAGS.knn_subtrain
            d = None
            y_train = None
            for j in range(train_iterations):
                start_idx = j * FLAGS.knn_subtrain
...
def eval_from_matrix_loo(train_features, train_labels):
    logging.log(logging.DEBUG, "Start computing LOO distance matrix with measure='{}'".format(FLAGS.knn_measure))
    start = time.time()
